...
import { prisma } from "..";
import { parseQuestionsCsv } from "../utils/questionsCsvParsing";
...
const validateReadyField = (requestData: AddQuestionRequestBody): boolean => {
  if (isFillBlankQuestion(requestData)) {
    ...
    const allAnswersMatchBlanks = requestData.answers?.every((answer) =>
      possibleAnswersIndices.includes(answer.blankIndex)
    );
...
const createQuestionsInLevel = async (levelId: string, csvPath: string, clearExisting: string = "false") => {
  try {
    ...
    type BaseQuestionType = {
      ...
      levelId: string;
      explanation: string;
      questionType: "MCQ" | "FILL_IN_BLANK" | "MATCHING";
    };

    type McqQuestionType = BaseQuestionType & {
      questionType: "MCQ";
      mcqAnswers: { value: string; isCorrect: boolean }[];
    };

    type FillInBlankQuestionType = BaseQuestionType & {
      questionType: "FILL_IN_BLANK";
      segments: { text: string; isBlank: boolean; order: number }[];
      answers: { value: string; blankIndex: number }[];
    };

    type MatchingQuestionType = BaseQuestionType & {
      questionType: "MATCHING";
      pairs: { leftItem: string; rightItem: string }[];
    };

    const exampleQuestionsData = parseQuestionsCsv(csvPath) as (
      | McqQuestionType
      | FillInBlankQuestionType
      | MatchingQuestionType
    )[];

    const questionsData = exampleQuestionsData.map((question) => ({
      ...question,
      levelId: level.id,
    }));
    ...
    for (const question of questionsData) {
      ...
      switch (question.questionType) {
        case "MCQ":
          ...
          const isMcqReady = (question: McqQuestionType) => {
            if (question.mcqAnswers.length < 4) return false;
