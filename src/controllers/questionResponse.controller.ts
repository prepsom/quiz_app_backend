...
type QuestionResponseRequestBody = MCQResponse | FillInBlankResponse | MatchingResponse;
...
const answerQuestionHandler = async (req: Request, res: Response) => {
    try {
        ...
        const responsePayload: Prisma.QuestionResponseUncheckedCreateInput = {
            ...
            responseData: 'selectedAnswerId' in responseData ? null : responseData as any,
            questionId: question.id,
            responderId: userId,
        };

        const questionResponse = existingResponse 
            ? await prisma.questionResponse.update({where: { id: existingResponse.id },data: {
                    ...responsePayload,
...
const validateResponse = async (responseData: QuestionResponseRequestBody, question: any) => {
...
const validateMCQResponse = (response: MCQResponse, question: any) => {
    ...
    const correctAnswer = question.MCQAnswers.find((answer: any) => answer.isCorrect);
...
const validateFillInBlankResponse = (response: FillInBlankResponse, question: any) => {
    const correctAnswers = question.BlankAnswers;
    let isCorrect = true;
    
    // Group correct answers by blank index
    const correctAnswersByIndex = correctAnswers.reduce((acc: any, answer: any) => {
...
const validateMatchingResponse = (response: MatchingResponse, question: any) => {
...
const calculatePoints = (isCorrect: boolean, difficulty: Difficulty, timeTaken: number): number => {
