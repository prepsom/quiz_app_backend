...
const addSubjectByGradeHandler = async (req: Request, res: Response) => {
  try {
    ...
    if (user.role === "TEACHER") {
      ...
      if (!teacherGrade) {
        res.status(401).json({
          success: false,
          message:
            "teacher doesn't teach this grade. unauthorized to add a subject",
        });
        return;
      }
    }
    ...
    const currentSubjectWithHighestPosition = await prisma.subject.findFirst({
      where: {},
    ...
    if (user.role === "TEACHER") {
      ...
      if (!teachesGrade) {
        res.status(401).json({
          success: false,
          message:
            "teacher doesn't teach this grade. unauthorized to delete a subject",
        });
        return;
      }
    }
