...
const containsCharFrom = (str: string, charSet: string): boolean => {
...
const generatePassword = (options: PasswordOptions = {}): string => {
  ...
  if (includeSpecials) chars += CHARS.special;
  ...
  const requirements = [];
  if (includeUppercase) requirements.push(CHARS.uppercase);
  if (includeLowercase) requirements.push(CHARS.lowercase);
  if (includeNumbers) requirements.push(CHARS.numbers);
  if (includeSpecials) requirements.push(CHARS.special);
